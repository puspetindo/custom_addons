...
class ExperienceListJs(models.Model):
    _name = 'experience.list.js'
    _description = 'Experience List JS - Daftar Order Information JS'
    ...
    def init(self):
        self._cr.execute("""
            DROP VIEW IF EXISTS experience_list_js CASCADE
        """)
        self._cr.execute("""
            CREATE OR REPLACE VIEW experience_list_js AS (
                SELECT 
                ...
                FROM (
                    ...
                    FROM sales_custom_job_item_js ji
                ) ji
                INNER JOIN sales_custom_oi_js oi ON ji.order_id = oi.id
                WHERE oi.status != 'cancel'
                  AND ji.rn = 1   -- hanya ambil equipment pertama tiap OI JS
                ORDER BY oi.id
            )
        """)
    ...
    def action_download_excel(self):
        ...
        # Title
        worksheet.merge_range('A1:K1', 'EXPERIENCE LIST JS', title_format)
        ...
        file_name = 'Experience_List_JS_%s.xlsx' % datetime.today().year
        attachment = self.env['ir.attachment'].create({
            'name': file_name,
            'type': 'binary',
            'datas': base64.b64encode(output.read()),
            'mimetype': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
            'res_model': 'experience.list.js',
            'res_id': False,
        })
